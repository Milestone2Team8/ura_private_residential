...
FEATURES_KEEP = ["area", "noOfUnits", "target_price", "marketSegment"]
FEATURES_CAT = ["marketSegment"]
...
def process_outliers_data(
    ...
    if keep_features is None:
        keep_features = FEATURES_KEEP
    if cat_features is None:
        cat_features = FEATURES_CAT
    df_copy = df_ura[keep_features].copy()
    num_features = list(df_copy.columns.difference(cat_features))
    ...
    missing_table = diagnose_missing_data(df_copy, "outlier detection data")
...
def plot_outliers_umap(
    ...
    x_processed, is_cat = process_outliers_data(
        df_outliers_sample,
        cat_imputer_strategy=CAT_IMPUTER_STRATEGY,
        num_imputer_strategy=NUM_IMPUTER_STRATEGY,
        cat_constant=CAT_CONSTANT,
...
def detect_outliers_generate_plots(df_ura, detectors=None):
    """
    ...
    df_copy.to_csv(
        "./src/data/output/clean_merged_with_outliers.csv", index=False
    )
