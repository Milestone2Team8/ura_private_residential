...
def extract_fetch_date(path):
    ...
    :param path: A pathlib.Path object representing the file path.
    :type path: Path

    :return: The extracted date if parsing succeeds, otherwise None.
    :rtype: d
    """
    try:
...
def clean_google_data(poi_type_list):
    ...
    :param poi_type_list: List of POI types to process (e.g., ['restaurant', 'school']).
    :type poi_type_list: list[str]

    :return: Combined and cleaned POI data with geometry and calculated fields.
    :rtype: geopandas.GeoDataFrame

    :raises ValueError: If no valid input files are found for any POI type.
    """
    combined_gdfs = []

    for poi_type in poi_type_list:
        input_files = sorted(
            Path("src/data/input").glob(f"raw_google_data_{poi_type}_*.csv")
        )
        ...
        df_delta = df_delta.join(
            df_min[["user_ratings_total"]], rsuffix="_min", how="left"
        )
        ...
        df_delta["delta_time"] = (
            files_with_dates[-1][1] - files_with_dates[0][1]
        ).days
